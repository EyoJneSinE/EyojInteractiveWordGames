...
class LearnedWordsCarouselViewHolder (
    private val binding: LearnedItemCarouselBinding,
    private val carouselClickListener: CarouselClickListener
) : RecyclerView.ViewHolder(binding.root) {
    fun bindListToViewPager(item: UIWordCard) = with(binding) {
        textLearnedWord.text = item.wordName
        ...
        val color =
            ContextCompat.getColor(cardLearnedWord.context, colorList[adapterPosition % colorList.size])
        cardLearnedWord.setCardBackgroundColor(ColorStateList.valueOf(color))
        cardLearnedWord.setOnClickListener {
